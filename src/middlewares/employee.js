...
module.exports = {
  check: function (req, res, next) {
    ...
    database.query(
      ...
      (errors, results, fields) => {
        if (results.length == 0) return res.redirect("/user/employee-data");
        else next();
      }
    );
  },
  ...
  storePhoto: function () {
    const storage = multer.diskStorage({
      destination: function (req, file, cb) {
        ...
        console.log("DIR exist : " + dirExist);

        if (!dirExist) fs.mkdirSync(dir, { recursive: true });
  ...
  restrictFile: function (req, res, next) {
    ...
    console.log("File Nip : " + req.params.fileName.split("_")[0]);
    const userNip = req.session.user.nip;
    const isAutheticated = photoNip == userNip;

    if (!isAutheticated) return res.send("Unauthorized.");
  ...
  getPhoto: function (req, res, next) {
    ...
    const dir = path.resolve(`assets/private/${nip}/${fileName}`);
