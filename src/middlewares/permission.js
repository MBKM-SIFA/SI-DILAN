...
module.exports = {
    getAll : function(req, res, next){
        database.query(
            `SELECT * FROM application WHERE app_type='Izin Belajar' AND nip=${req.session.user.nip}`,
            (err, row, fields) => {
                if (err)
                return res.send(err);
                else
    ...
    resume :function(req, res, next){
        ...
        return res.render(`user/permission-phase${last_phase}`);
    },
    ...
    upload : function (){
        ...
        const final = upload.fields([
            ...
            { name : 'Surat Pernyataan' , maxCount : 1},
            { name : 'Surat Permohonan' , maxCount : 1},
            { name : 'SK Terakhir' , maxCount : 1},
            { name : 'P2KP' , maxCount : 1},
            { name : 'Uraian Tugas' , maxCount : 1},
            { name : 'SK Kelas Reguler' , maxCount : 1},
            { name : 'Jadwal Kelas Reguler' , maxCount : 1},
            { name : 'Akreditasi' , maxCount : 1},
            { name : 'SK PNS' , maxCount : 1}
        ]);
